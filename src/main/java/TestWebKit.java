...
public class TestWebKit {
    private static final String TITLE = "WebKit Test Application";
        ...
        SwingUtilities.invokeLater(new Runnable() {
            public void run() {
                ...
                webView.addProgressListener(new ProgressListener() {
                    ...
                    public void loadingFailed(WebView pView, WebFrame pFrame) {
                        if (pFrame.equals(pView.mainFrame())) {
                            addressField.setInProgress(false);
                            addressField.setBackground(new Color(0xFFcccc));
                        }
                    }
                    ...
                    public void actionPerformed(ActionEvent event) {
                        ...
                        try {
                            ...
                            if (uri.getScheme() == null) {
                                if (url.startsWith("/")) {
                                    uri = new URI("file", "//localhost" + url, null);
                                }
                                else {
...
//                webView.setMainFrameURL(URI.create("http://agap07:8080/pes/"));
    ...
    public static class JWebView extends CocoaComponent {
        ...
        public class WebFrameLoadDelegate {
            ...
            // Error Messages
            public void webView_didFailProvisionalLoadWithError_forFrame(final WebView view, final /*NSError*/ NSObject error, final WebFrame frame) {
                System.err.println("TestWebKit$JWebView$WebFrameLoadDelegate.webView_didFailProvisionalLoadWithError_forFrame: " + error);
                SwingUtilities.invokeLater(new Runnable() {
                    public void run() {
                        for (ProgressListener listener : mProgressListeners) {
                            listener.loadingFailed(view, frame);
                        }
                    }
                });
            }
    ...
    static interface ProgressListener {
        ...
        // TODO: Do we need two fail callbacks? Error?
        void loadingFailed(WebView pView, WebFrame pFrame);
    }
